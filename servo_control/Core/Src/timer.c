...
struct _HardwareTimer {
	TIM_TypeDef *TIMx;
	uint16_t PrescaleValue;	// prescaler value
	uint32_t ARRValue; 		// prescaler value
	uint8_t oneShotMode; 		// TRUE or FALSE
	uint32_t MaskAPB2ENR;		// mask to enable RCC APB2 bus registers
	uint32_t MaskAPB1ENR;		// mask to enable RCC APB1 bus registers
	uint32_t MaskAHBENR;		// mask to enable RCC AHB bus registers
	uint32_t MaskCR1; 			// mask to apply to CR1 (control reg 1)
	uint32_t MaskDIER; 		// mask to apply to DIER (DMA/interrupt enable register)
	void (*timer_completion_function)(); 	// the the code block to be run when the interrupt is flagged
};

HardwareTimer TIM2_init = {
		.TIMx = TIM2,
		.PrescaleValue = 1, 				// default prescaler value
		.ARRValue = 1000,					// overflow value
		.oneShotMode = FALSE, 				// not in one-shot mode
		.MaskAPB2ENR = 0x00, 				// bit to enable APB2 bus
		.MaskAPB1ENR = RCC_APB1ENR_TIM2EN, 	// bit to enable APB1 bus
		.MaskAHBENR = 0x00, 				// bit to enable for AHB bus
		.MaskCR1 = TIM_CR1_URS, 			// bit for CR1 reg; interrupt only flagged at over/underflow
		.MaskDIER = TIM_DIER_UIE, 			// enable update interrupt
		.timer_completion_function = 0x00	//
};

HardwareTimer TIM4_init = {
		.TIMx = TIM4,
		.PrescaleValue = 1, 					// default prescaler value
		.ARRValue = 1000, 				// default overflow value
		.oneShotMode = FALSE, 				// not in one-shot mode
		.MaskAPB2ENR =0x00, 				// bit to enable APB2 bus
		.MaskAPB1ENR = RCC_APB1ENR_TIM4EN, // bit to enable APB1 bus
		.MaskAHBENR = 0x00, 				// bit to enable for AHB bus
		.MaskCR1 = TIM_CR1_URS, 		// bit for CR1 reg; interrupt only flagged at over/underflow
		.MaskDIER = TIM_DIER_UIE, 		// enable update interrupt
		.timer_completion_function = 0x00
